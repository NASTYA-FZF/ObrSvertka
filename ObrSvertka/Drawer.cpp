...
void Drawer::Create(HWND hWnd)
{
	...
	dc = wnd->GetDC();
	dc_forOnPaint = wnd->GetDC();

	// Создаем буфер-контекст.
	memDC.CreateCompatibleDC(dc);
	//memDC_forOnPaint.CreateCompatibleDC(dc_forOnPaint);
	// Создаем растр для контекста рисования.
	bmp.CreateCompatibleBitmap(dc, frame.Width(), frame.Height());
	//bmp_forOnPaint.CreateCompatibleBitmap(dc_forOnPaint, frame.Width(), frame.Height());
	// Выбираем растр для использования буфер-контекстом.
	memDC.SelectObject(&bmp);
	//memDC_forOnPaint.SelectObject(&bmp_forOnPaint);
	init = true;
}
...
void Drawer::DrawPoints(vector<double>& data, CString Ox, CString Oy, double data_x_max, double step_x, bool flag)
{
	if (!flag)
	{
		if (!init) return;
	...
	else
	{
		...
		{
			//memDC.LineTo(convert_range_d((double)i /*/ s.Fd*/ * step_x, actual_right, actual_left, data_x_max, data_x_min), y[i]);
			double x = convert_range_d((double)i /*/ s.Fd*/ * step_x, actual_right, actual_left, data_x_max, data_x_min);
			memDC.Ellipse(x - 3, y[i] + 3, x + 3, y[i] - 3);
		}
		...
		for (int i = 0; i < grid_size / 2 + 1; i++)
		{
			...
			memDC.TextOutW(actual_left - 3 * left_keys_padding, actual_bottom - (double)i * actual_height / (grid_size / 2) - bottom_keys_padding, str);
		}
		//str.Format(L"x");
		memDC.TextOutW(actual_left - 10, actual_top - 30, Oy);
...
void Drawer::DrawTwoSig(vector<double>& data, vector<double>& data1, CString Ox, CString Oy, double data_x_max, double step_x)
{
	if (!init) return;
	...
	CPen three_pen(PS_SOLID, 2, RGB(255, 105, 0));
	CFont font;
	font.CreateFontW(18, 0, 0, 0,
		FW_DONTCARE,
		FALSE,				// Курсив
	...
	for (unsigned int i = 1; i < y.size(); i++)
	{
		memDC.LineTo(convert_range_d((double)i * step_x, actual_right, actual_left, data_x_max, data_x_min), y[i]);
	}
...
void Drawer::ReDraw()
{
	dc_forOnPaint->BitBlt(0, 0, frame.Width(), frame.Height(), &memDC, 0, 0, SRCCOPY);
}

void Drawer::My_Draw(vector<double>& data, CString Ox, CString Oy, double data_x_max, double step_x, bool flag)
{
	if (!flag)
	{
		if (!init) return;
...
void Drawer::PicPoints(vector<double>& data, CString Ox, CString Oy, double data_x_max, double step_x)
{
	if (!init) return;
	...
	for (unsigned int i = 1; i < y.size(); i++)
	{
		//memDC.LineTo(convert_range_d((double)i /*/ s.Fd*/ * step_x, actual_right, actual_left, data_x_max, data_x_min), y[i]);
		double x = convert_range_d((double)i /*/ s.Fd*/ * step_x, actual_right, actual_left, data_x_max, data_x_min);
		memDC.Ellipse(x - 5, y[i] + 5, x + 5, y[i] - 5);
	}
